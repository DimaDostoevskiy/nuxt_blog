...
Попадать в выдачу поисковиков по специфичным темам, конкретного бизнеса. Но при этом робот должен иметь возможность
анализировать любую информацию с главного ресурса.

## Решение

Развернуть как ещё одну страницу сайта, отдельное web-приложение на Nuxt для SEO оптимизации.
...
- Если пользователь авторизован на основном ресурсе `mysite.pro/`, при переходе на `mysite.pro/blog/` в запрос
  добавляется query-параметр `token` (настроен vue-router).
- Если токен валидный: делаем запрос на внешний ресурс (env: `EXTERNAL_AUTH_URL`), который меняет `token` на
  пользователя (env EXTERNAL_AUTH_URL).
